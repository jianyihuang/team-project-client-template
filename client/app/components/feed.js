...
export default class Feed extends React.Component {
    ...
    render() {
        if (!this.props.isRequestPage) {
            return (
                <div>
                    <PostUpdateEntry onPost= {(postContents) => this.onPost(postContents)}/> {this.state.list_of_feeditems.map((feedItem) => {
                        // console.log('Feed Items Data:' + JSON.stringify(feedItem));
                        return (<FeedItem key={feedItem._id} data={feedItem} type={this.props.type} refresh= {()=> this.refresh()} current_user={this.state.current_user}/>)
                    })}
                </div>
            )
        } else {
